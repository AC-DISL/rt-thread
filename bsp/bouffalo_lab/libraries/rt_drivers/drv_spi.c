...
static ATTR_NOCACHE_NOINIT_RAM_SECTION rt_uint8_t
    dma_tx_buf[BSP_SPI_TX_DMA_NOCACHE_BUFSIZE];
#endif

#if defined(BSP_SPI_RX_USING_DMA) && defined(BSP_SPI_RX_DMA_NOCACHE_BUFSIZE)
static ATTR_NOCACHE_NOINIT_RAM_SECTION rt_uint8_t
    dma_rx_buf[BSP_SPI_RX_DMA_NOCACHE_BUFSIZE];
#endif
...
static rt_err_t spi_configure(struct rt_spi_device *device,
  ...
  bflb_dma_channel_irq_attach(bl_spi->dma_tx, spi_bl_spi_dma_tx_isr,
                              (void *)bl_spi);
#endif
...
#if defined(BSP_SPI_TX_USING_DMA)
static rt_err_t _bl_spi_dma_tx(struct bl_device_spi *bl_spi, rt_uint8_t *src,
                               rt_size_t length) {
  rt_err_t result;
...
static rt_err_t _spi_dma_xfer_tx(struct rt_spi_device *device,
                                 struct rt_spi_message *message) {
  rt_err_t result = RT_EOK;
  rt_uint8_t *src = (rt_uint8_t *)message->send_buf;
  rt_size_t length = message->length;
  struct bl_device_spi *bl_spi =
      (struct bl_device_spi *)device->bus->parent.user_data;

#if defined(NOCACHE_BUFSTART) && defined(NOCACHE_BUFSIZE)
  if ((message->send_buf < NOCACHE_BUFSTART) ||
      (message->send_buf > (NOCACHE_BUFSTART + NOCACHE_BUFSIZE))) {
    if (length <= BSP_SPI_TX_DMA_NOCACHE_BUFSIZE) {
      ...
      while (length > 0) {
        if (length >= BSP_SPI_TX_DMA_NOCACHE_BUFSIZE) {
          ...
          result = _bl_spi_dma_tx(bl_spi, dma_tx_buf,
                                  BSP_SPI_TX_DMA_NOCACHE_BUFSIZE);
          if (result != RT_EOK)
            break;
...
#if defined(BSP_SPI_RX_USING_DMA)
static rt_err_t _bl_spi_dma_rx(struct bl_device_spi *bl_spi, rt_uint8_t *dst,
                               rt_size_t length) {
  rt_err_t result;
...
static rt_err_t _spi_dma_xfer_rx(struct rt_spi_device *device,
                                 struct rt_spi_message *message) {
  rt_err_t result = RT_EOK;
...
#if defined(NOCACHE_BUFSTART) && defined(NOCACHE_BUFSIZE)
  if ((message->recv_buf < NOCACHE_BUFSTART) ||
      (message->recv_buf > (NOCACHE_BUFSTART + NOCACHE_BUFSIZE))) {
    if (length <= BSP_SPI_RX_DMA_NOCACHE_BUFSIZE) {
      ...
      while (length > 0) {
        if (length >= BSP_SPI_RX_DMA_NOCACHE_BUFSIZE) {
          result = _bl_spi_dma_rx(bl_spi, dma_tx_buf,
                                  BSP_SPI_RX_DMA_NOCACHE_BUFSIZE);
          if (result != RT_EOK)
            break;
...
rt_err_t rt_hw_spi_device_attach(const char *bus_name, const char *device_name,
  ...
  ret = rt_spi_bus_attach_device(spi_device, device_name, bus_name,
                                 (void *)cs_pin);
  if (ret != RT_EOK) {
...
int rt_hw_spi_init(void) {
  ...
  bflb_gpio_init(gpio, SPI_SCK_PIN,
                 GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN |
                     GPIO_DRV_1);
  bflb_gpio_init(gpio, SPI_MISO_PIN,
                 GPIO_FUNC_SPI0 | GPIO_ALTERNATE | GPIO_PULLUP | GPIO_SMT_EN |
                     GPIO_DRV_1);
  ...
  rt_hw_spi_device_attach("spi0", "spi0_dev2", GPIO_PIN_20); // mag
  rt_hw_spi_device_attach("spi0", "spi0_dev3", GPIO_PIN_28); // bar
