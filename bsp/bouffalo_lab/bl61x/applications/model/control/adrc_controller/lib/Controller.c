...
void Controller_step(void)
{
   ...
   *  Inport: '<Root>/INS_Out'
   */
  rtb_DiscreteTimeIntegrator_h = csi_sin_f32(Controller_U.INS_Out.phi);

  /* Trigonometry: '<S83>/Cos1' incorporates:
   *  Inport: '<Root>/INS_Out'
   */
  rtb_Integrator13 = csi_cos_f32(Controller_U.INS_Out.theta);
   ...
   *  Inport: '<Root>/INS_Out'
   */
  rtb_Integrator12 = csi_cos_f32(Controller_U.INS_Out.phi);
  ...
  if (Controller_U.FMS_Out.ctrl_mode == 2) {
    ...
    if ((Controller_U.FMS_Out.cmd_mask & 1) > 0) {
      ...
      rtb_Multiply2_f_idx_0 = rtb_Saturation1_idx_0 - csi_sin_f32
   ...
   */
  rtb_Add3_c = csi_cos_f32(Controller_U.INS_Out.phi) * csi_cos_f32
